		...
		<article id="comment-<?php comment_ID(); ?>" class="clearfix">
			<div class="comment-author vcard row-fluid clearfix">
				...
				<div class="span10 comment-text">
					<?php printf(__('<h4>%s</h4>','bonestheme'), esc_url(get_comment_author_link())) ?>
					<?php esc_url(edit_comment_link(__('Edit','bonestheme'),'<span class="edit-comment btn btn-small btn-info"><i class="icon-white icon-pencil"></i>','</span>')) ?>
                    ...
                    <time datetime="<?php echo comment_time('Y-m-j'); ?>"><a href="<?php echo htmlspecialchars( esc_url(get_comment_link( $comment->comment_ID )) ) ?>"><?php comment_time('F jS, Y'); ?> </a></time>
...
function list_pings($comment, $args, $depth) {
       $GLOBALS['comment'] = $comment;
?>
        <li id="comment-<?php comment_ID(); ?>"><i class="icon icon-share-alt"></i>&nbsp;<?php esc_url(comment_author_link()); ?>
<?php 

}

// Only display comments in comment count (which isn't currently displayed in vixo-bootstrap, but i'm putting this in now so i don't forget to later)
...
class description_walker extends Walker_Nav_Menu
{
      function start_el(&$output, $item, $depth, $args)
      {
			...
			$class_names = ' class="'. esc_attr( $class_names ) . '"';
           	...
           	$attributes .= ! empty( $item->url )        ? ' href="'   . esc_url(esc_attr( $item->url        )) .'"' : '';
...
{ 
    ...
    wp_register_style( 'vixo-bootstrap', get_template_directory_uri() . '/style.css', array(), '1.0', 'all' );
    
    wp_enqueue_style( 'bootstrap' );
    wp_enqueue_style( 'bootstrap-responsive' );
    wp_enqueue_style( 'vixo-bootstrap');
}
  ...
  $theme_options_styles = '';
      ...
      if( $bootstrap_theme && $use_theme ){
        if( $bootstrap_theme == 'default' ){}
          ...
          echo '<link rel="stylesheet" href="' . esc_url(get_template_directory_uri()) . '/admin/themes/' . $bootstrap_theme . '.css">';
        }
      }
